...
struct TBoundingBox
{
    ...
    BoundingBox3f getOctChild(int childIndex) const
    {
        if (childIndex < 0 || childIndex >= 8)
            throw NoriException("OctreeVisitor::visitChild: invalid child index");
        Point3f center = (min + max) * 0.5f;
        BoundingBox3f childBbox;

        // X dimension (bit 0)
        if (childIndex & 1)
        {
            childBbox.min.x() = center.x();
            childBbox.max.x() = max.x();
        }
        else
        {
            childBbox.min.x() = min.x();
            childBbox.max.x() = center.x();
        }

        // Y dimension (bit 1)
        if (childIndex & 2)
        {
            childBbox.min.y() = center.y();
            childBbox.max.y() = max.y();
        }
        else
        {
            childBbox.min.y() = min.y();
            childBbox.max.y() = center.y();
        }

        // Z dimension (bit 2)
        if (childIndex & 4)
        {
            childBbox.min.z() = center.z();
            childBbox.max.z() = max.z();
        }
        else
        {
            childBbox.min.z() = min.z();
            childBbox.max.z() = center.z();
        }

        return childBbox;
    }

    void setOctChild(int childIndex)
    {
        if (childIndex < 0 || childIndex >= 8)
            throw NoriException("OctreeVisitor::visitChild: invalid child index");
        Point3f center = (min + max) * 0.5f;
